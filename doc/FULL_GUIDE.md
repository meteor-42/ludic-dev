...
### 2.1. Сервис событий (Event Provider)

В текущей архитектуре `ludic-dev` является сервисом управления кошельками и ставками (Wallet & Betting Service). Управление спортивными событиями вынесено во внешний контур.

**Особенности интеграции:**
1.  **Внешний источник данных:** Коэффициенты и события поступают от внешнего провайдера (Event Service / Odds Provider).
2.  **Денормализация данных:** При создании ставки информация о событии (`eventName`, `odds`, `selection`) копируется в таблицу `bets`. Это гарантирует историческую целостность данных — даже если событие исчезнет из фида провайдера, информация о ставке пользователя останется неизменной.
3.  **Отсутствие таблицы Events:** В базе данных `ludic-dev` нет таблицы событий, так как сервис не является "владельцем" этих данных, а лишь потребителем в момент заключения пари.
...
Хранит текущий баланс виртуальной валюты для каждого пользователя.

| Название столбца | Тип данных | Ограничения (Constraints) | Описание |
| :--- | :--- | :--- | :--- |
| `id` | `SERIAL` | `PRIMARY KEY` | Уникальный идентификатор записи |
| `user_id` | `VARCHAR(255)` | `NOT NULL`, `UNIQUE` | ID пользователя из PocketBase (Внешний ключ) |
| `balance` | `DECIMAL(12, 2)` | `NOT NULL`, `DEFAULT 1000.00` | Текущий баланс средств |
| `created_at` | `TIMESTAMP` | `NOT NULL`, `DEFAULT NOW()` | Дата создания кошелька |
| `updated_at` | `TIMESTAMP` | `NOT NULL`, `DEFAULT NOW()` | Дата последнего изменения баланса |

### Таблица: `bets` (Ставки)
Хранит информацию о всех ставках, сделанных пользователями.

| Название столбца | Тип данных | Ограничения (Constraints) | Описание |
| :--- | :--- | :--- | :--- |
| `id` | `SERIAL` | `PRIMARY KEY` | Уникальный номер ставки |
| `user_id` | `VARCHAR(255)` | `NOT NULL` | ID пользователя, сделавшего ставку |
| `event_name` | `TEXT` | `NOT NULL` | Название события (напр. "Real Madrid vs Barcelona") |
| `selection` | `TEXT` | `NOT NULL` | Выбор пользователя (напр. "Победа 1") |
| `odds` | `DECIMAL(6, 2)` | `NOT NULL` | Коэффициент ставки на момент заключения пари |
| `amount` | `DECIMAL(12, 2)` | `NOT NULL` | Сумма ставки |
| `potential_win` | `DECIMAL(12, 2)` | `NOT NULL` | Возможный выигрыш (`amount` * `odds`) |
| `status` | `VARCHAR(20)` | `NOT NULL`, `DEFAULT 'pending'` | Статус: `pending` (ожидание), `won` (выигрыш), `lost` (проигрыш) |
| `result` | `VARCHAR(20)` | `NULLABLE` | Результат матча (заполняется после завершения) |
| `created_at` | `TIMESTAMP` | `NOT NULL`, `DEFAULT NOW()` | Дата и время совершения ставки |
| `settled_at` | `TIMESTAMP` | `NULLABLE` | Дата и время расчета ставки |
